// T O D O
//
// AddClass must create certificate
package main
...
<fieldset>
	<button style="padding: 10px 5px 7px 5px;" onclick="window.location.href='/editcert?class={{.Class}}'" title="Edit class certificate"><svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" fill="currentColor" class="bi bi-file-richtext" viewBox="0 0 16 16">
...
func createNewClass() int {
	...
	_, err := DBH.Exec(sqlx)
	checkerr(err)
	sqlx = fmt.Sprintf("INSERT OR IGNORE INTO certificates (Class,Title,html,options,image) SELECT %v,'Class %v',html,options,image FROM certificates WHERE Class=0", cls, cls)
	_, err = DBH.Exec(sqlx)
	checkerr(err)
	return cls
}
