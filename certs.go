...
func edit_certificate(w http.ResponseWriter, r *http.Request) {
	...
	if r.FormValue("class") != "" {
		sqlxx += " WHERE Class=" + strconv.Itoa(class)
	}
	html := getStringFromDB(sqlxx, getStringFromDB(sqlx, ""))
	...
	fmt.Fprint(w, `<div class="certedit">`)
	fmt.Fprint(w, `<form action="/savecert">`)
	//fmt.Fprintf(w, `<input type="hidden" name="class" value="%v">`, class)
	fmt.Fprint(w, `<label for="Class">Class</label> `)
	fmt.Fprintf(w, `<select id="Class" name="class" onchange="window.location.href='/editcert?class='+this.value;">`)
	for i := range classes {
		sel := ""
		if classes[i].Class == class {
			sel = "selected"
		}
		fmt.Fprintf(w, `<option value="%v" %v>%v</options>`, classes[i].Class, sel, classes[i].BriefDesc)
	}
	fmt.Fprint(w, `</select>`)
...
func print_certs(w http.ResponseWriter, r *http.Request) {
	...
	sqlx += ",ifnull(FinishPosition,0),ifnull(TeamID,0),TotalPoints,Class"
	sqlx += " FROM entrants "
	if r.FormValue("all") == "" {
	...
	for rows.Next() {
		...
		var pf string
		var pl string
		err = rows.Scan(&cf.Bike, &rf, &rl, &pf, &pl, &cf.Distance, &cf.EntrantID, &cf.Place, &team, &cf.Points, &cf.Class)
		checkerr(err)
		...
		html := getStringFromDB(fmt.Sprintf("SELECT ifnull(html,'') FROM certificates WHERE Class=%v", cf.Class), "")
...
func save_certificate(w http.ResponseWriter, r *http.Request) {
	...
	sqlx := "INSERT OR REPLACE INTO certificates(html,EntrantID,Class) VALUES(?,?,?)"
